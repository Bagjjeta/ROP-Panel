...
class ROP_Panel_Profile_Editor {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_rop_delete_company_logo', array($this, 'delete_company_logo'));
    ...
    private function get_user_company_data($user_id) {
        $rop_email = get_user_meta($user_id, 'rop_email', true);
        $rop_website = get_user_meta($user_id, 'rop_website', true);

        $email = $rop_email;
        $website = $rop_website;

        if (empty($email) && function_exists('um_user')) {
            um_fetch_user($user_id);

            $um_email = um_user('user_email');
            if (empty($um_email)) {
                $um_email = um_user('email');
            }
            if (empty($um_email)) {
                $um_email = um_user('business_email');
            }
            if (empty($um_email)) {
                $um_email = um_user('company_email');
            }
        ...
        if (empty($website) && function_exists('um_user')) {
            um_fetch_user($user_id);

            $um_website = um_user('user_url');
            if (empty($um_website)) {
                $um_website = um_user('website');
            }
            if (empty($um_website)) {
                $um_website = um_user('company_website');
            }
            if (empty($um_website)) {
                $um_website = um_user('business_website');
            }
            if (empty($um_website)) {
                $um_website = um_user('url');
            }
        ...
        if (empty($website)) {
            $user_data = get_userdata($user_id);
            if ($user_data && !empty($user_data->user_url)) {
